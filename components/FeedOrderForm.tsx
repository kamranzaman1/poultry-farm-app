...
import type { FeedOrderData, ChicksReceivingData, SelectedFarmCycleDetails, User } from '../types';
...
const FeedOrderForm: React.FC<FeedOrderFormProps> = ({ 
  ...
  const handleHeaderChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    const { name, value } = e.target;
     setData(prevData => {
        const oldHeaderValue = prevData[name as keyof FeedOrderData] as string;
        let newItems = prevData.items;

        // When a header field changes, propagate it to items that are either empty or matched the old header value
        if (name === 'deliveryDate') {
            newItems = prevData.items.map(item => ({
                ...item,
                deliveryDate: !item.deliveryDate || item.deliveryDate === oldHeaderValue ? value : item.deliveryDate,
            }));
        } else if (name === 'feedMillNo') {
            newItems = prevData.items.map(item => ({
                ...item,
                stoNo: !item.stoNo || item.stoNo === oldHeaderValue ? value : item.stoNo,
            }));
        }
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    const currentInput = e.currentTarget as (HTMLInputElement | HTMLSelectElement);
    ...
    const totalCols = 4; // deliveryDate, stoNo, feedType, quantity
  ...
  const handlePaste = (e: React.ClipboardEvent<HTMLTableSectionElement>) => {
    ...
    const colMap = ['deliveryDate', 'stoNo', 'feedType', 'quantity'];
        ...
        setData(prevData => {
            ...
            pastedData.forEach((cellValue, index) => {
                ...
                if (targetRow < newItems.length) {
                    if (startCol === 0) newItems[targetRow].deliveryDate = cellValue;
                    if (startCol === 1) newItems[targetRow].stoNo = cellValue;
                    if (startCol === 2) {
                    ...
                    if (startCol === 3) newItems[targetRow].quantity = cellValue;
                }
            });
            return { ...prevData, items: newItems };
        });
    ...
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md">
           ...
           <button type="button" onClick={isEditing ? onCancelEdit : onBack} className="p-1 rounded-full text-gray-500 hover:bg-gray-200 hover:text-gray-800 transition-colors" aria-label="Go back">
            ...
            <div className="text-center flex-grow">
                ...
                <div className="flex justify-center items-center gap-2">
                    ...
                    {isEditing && currentUser.role === 'Admin' && (initialData as any).meta && (
                        <div className="relative group mt-2">
                            <ClockIcon />
                            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-xs p-2 bg-gray-800 text-white text-xs rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-10">
                                Created by: <strong>{(initialData as any).meta.createdBy}</strong> at {new Date((initialData as any).meta.createdAt).toLocaleString()}
                                <br/>
                                Last updated by: <strong>{(initialData as any).meta.updatedBy}</strong> at {new Date((initialData as any).meta.updatedAt).toLocaleString()}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-4 border-y py-4">
        ...
        <div className="lg:col-span-2">
          <label htmlFor="feedMillNo" className="block text-sm font-medium text-gray-700">STO NO. (all houses):</label>
        ...
        <table className="min-w-full divide-y divide-gray-200 border">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r">STO NO.</th>
          ...
          <tbody className={`bg-white divide-y divide-gray-200 ${isLocked ? 'bg-gray-50' : ''}`} onPaste={handlePaste}>
            {data.items.map((item, i) => (
              <tr key={i}>
                ...
                <td className="px-2 py-1 whitespace-nowrap border-r">
                  <input type="text" name="stoNo" value={item.stoNo || ''} disabled={isLocked} onChange={(e) => handleItemChange(i, e)} onKeyDown={handleKeyDown} data-row-index={i} data-col-index={1} className="w-full border-none focus:ring-0 bg-transparent disabled:bg-transparent" />
                </td>
                <td className="px-2 py-1 whitespace-nowrap border-r">
                  <input type="number" name="age" value={item.age} readOnly disabled className="w-full border-none focus:ring-0 bg-gray-100" />
                </td>
                <td className="px-2 py-1 whitespace-nowrap border-r">
                   <select name="feedType" value={item.feedType} disabled={isLocked} onChange={(e) => handleItemChange(i, e)} onKeyDown={handleKeyDown} data-row-index={i} data-col-index={2} className="w-full border-none focus:ring-0 disabled:bg-transparent">
                    {feedTypes.map(ft => <option key={ft.value} value={ft.value}>{ft.label}</option>)}
                   </select>
                </td>
                <td className="px-2 py-1 whitespace-nowrap">
                  <input type="number" step="0.01" name="quantity" value={item.quantity} disabled={isLocked} onChange={(e) => handleItemChange(i, e)} onKeyDown={handleKeyDown} data-row-index={i} data-col-index={3} className="w-full border-none focus:ring-0 disabled:bg-transparent" />
                </td>
              </tr>
            ))}
          </tbody>
           <tfoot className="bg-gray-50">
                <tr>
                    <td colSpan={5} className="px-4 py-2 text-right font-bold text-gray-700 border-t-2 border-b">Grand Total:</td>
      ...
      <PreviewModal isOpen={isPreviewOpen} onClose={() => setIsPreviewOpen(false)} title={`Feed Order: ${farmName}`}>
        <div className="space-y-6 text-sm">
            <div className="space-y-2">
                ...
                <div className="flex justify-between"><span className="text-gray-600">Header Delivery Date:</span> <span className="font-medium text-gray-900">{data.deliveryDate || 'N/A'}</span></div>
                <div className="flex justify-between"><span className="text-gray-600">Header STO No:</span> <span className="font-medium text-gray-900">{data.feedMillNo || 'N/A'}</span></div>
            </div>
                        ...
                        {itemsWithQuantity.map((item, index) => (
                            <div key={index} className="p-3 bg-gray-50 rounded-md border">
                                ...
                                <div className="mt-1 text-xs space-y-1">
                                    ...
                                    <div className="flex justify-between"><span className="text-gray-600">STO No.:</span> <span className="font-medium text-gray-900">{item.stoNo || 'N/A'}</span></div>
