...
interface SubmittedFeedOrdersListProps {
  ...
  onConfirmDelivery: (orderId: string, actualDeliveryDate: string, deliveryData: FeedDeliveryRecordData) => void;
  currentUser: User;
  onBack: () => void;
...
const SubmittedFeedOrdersList: React.FC<SubmittedFeedOrdersListProps> = ({ orders, onConfirmDelivery, currentUser, onBack, onEdit }) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [selectedOrder, setSelectedOrder] = useState<SubmittedFeedOrder | null>(null);

  const canDeliver = currentUser.role === 'Admin' || currentUser.role === 'Supervisor';
  ...
  const handleConfirm = (deliveryDate: string, deliveryData: FeedDeliveryRecordData) => {
    if (selectedOrder) {
      onConfirmDelivery(selectedOrder.id, deliveryDate, deliveryData);
      handleCloseModal();
    }
  };
